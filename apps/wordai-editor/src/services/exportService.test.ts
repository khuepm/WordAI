...
import { exportDocx, exportMarkdown, importFile } from './exportService';
import { syncDocument } from './auraBrainManager';
import type { AuraIntentDocument, AuraImportResult } from '../types/auraDocument';
...
function makeAuraDocument(overrides: Partial<AuraIntentDocument> = {}): AuraIntentDocument {
  return {
    id: 'intent-1',
    intent_name: 'Imported Intent',
    content: [{ type: 'paragraph', text: 'Imported text', inline: [{ kind: 'text', text: 'Imported text' }] }],
    version: 1,
...
describe('exportService', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    randomUUIDSpy = vi.spyOn(crypto, 'randomUUID').mockReturnValue(NEW_IMPORT_ID);
  });
  ...
  it('returns cancelled when Markdown save dialog is cancelled', async () => {
    ...
    await expect(exportMarkdown(makeDocument())).resolves.toEqual({ status: 'cancelled' });
  ...
  it('imports a file without Aura tag as a new AuraBrain intent', async () => {
    ...
    expect(mockSyncDocument).toHaveBeenCalledWith(expect.objectContaining({ id: NEW_IMPORT_ID }), 'import');
    expect(onOpenIntent).toHaveBeenCalledWith(expect.objectContaining({ id: NEW_IMPORT_ID }));
  });
  ...
  it('updates an existing intent when imported Aura tag conflicts and user chooses update', async () => {
    ...
    mockInvoke.mockImplementation(async (cmd) => {
      ...
      if (cmd === 'get_intent') return makeAuraDocument({ id: 'intent-1', intent_name: 'Existing Intent' });
      return null;
    });
    ...
    expect(mockSyncDocument).toHaveBeenCalledWith(expect.objectContaining({ id: 'intent-1' }), 'import');
    expect(onOpenIntent).toHaveBeenCalledWith(expect.objectContaining({ id: 'intent-1' }));
  });
  ...
  it('creates a new intent when imported Aura tag conflicts and user chooses create new', async () => {
    ...
    const result = await importFile({ onConflict: vi.fn().mockResolvedValue('create_new') });
